...
init() ->
    ...
    LoaderFun = fun
        ...
        A(X) ->
            io:format("error: schema ~p not found", [X]),
            'undefined'
    end,
...
validate_cname_hosts(CNAME, Hosts) ->
    lists:flatten(
        lists:map(
            fun(H) -> validate_cname_host(CNAME, H) end,
            Hosts
        )
    ).
...
fail_cname(#state{
    ...
    [
        {"Validate badly formed host fails domains validation",
            ?_assertMatch(
                {'error', [
                    {'data_invalid', _, 'no_extra_properties_allowed', _, _},
                    {'data_invalid', _, 'no_extra_properties_allowed', _, _}
                ]},
                kz_json_schema:validate(
                    DomainsSchema,
...
validate_a_record_hosts(A_RECORD, Hosts) ->
    lists:flatten(
        lists:map(
            fun(H) -> validate_a_record_host(A_RECORD, H) end,
            Hosts
        )
    ).
...
validate_a_record_host(A_RECORD, Host) ->
    ...
    [{"Verify whitelabel host", ?_assert('nomatch' =/= binary:match(WhitelabelHost, ?DOMAIN))}].
...
validate_naptr_hosts(NAPTR, Hosts) ->
    lists:flatten(
        lists:map(
            fun(H) -> validate_naptr_host(NAPTR, H) end,
            Hosts
        )
    ).
...
validate_srv_hosts(SRV, Hosts) ->
    lists:flatten(
        lists:map(
            fun(H) -> validate_srv_host(SRV, H) end,
            Hosts
        )
    ).
