...
-type error_handler_fun() :: fun(
    (jesse_error:error_reason(), [jesse_error:error_reason()], non_neg_integer()) -> [
        jesse_error:error_reason()
    ]
).
-type schema_loader_fun() :: fun(
    (kz_term:ne_binary()) -> {'ok', kz_json:object()} | kz_json:object() | 'not_found'
).
-type setter_fun() :: fun(
    (kz_json:path(), kz_json:json_term(), kz_json:object()) -> kz_json:object()
).
-type getter_fun() :: fun(
    (kz_json:path(), kz_json:object(), jesse:json_term()) -> jesse:json_term()
).
...
load(Schema) ->
    load(kz_term:to_binary(Schema)).
-endif.

-spec fload(kz_term:ne_binary() | string()) ->
    {'ok', kz_json:object()}
    | {'error', 'not_found'}.
fload(<<"./", Schema/binary>>) ->
    fload(Schema);
...
fload(Schema) ->
    fload(kz_term:to_binary(Schema)).
...
fload_file(SchemaPath) ->
    ...
    case file:read_file(SchemaPath) of
        {'ok', SchemaJSON} ->
            {'ok', kz_json:insert_value(<<"id">>, Schema, kz_json:decode(SchemaJSON))};
        Error ->
            Error
    end.
...
maybe_log_diff('undefined', _Name, _JustSchemaDiff, _JustDiskDiff) ->
    'ok';
maybe_log_diff(Verbosity, SchemaName, JustSchemaDiff, JustDiskDiff) ->
    case {kz_json:is_empty(JustSchemaDiff), kz_json:is_empty(JustDiskDiff)} of
        ...
        _ when Verbosity =:= 'schema' ->
            io:format(
                "'~s' differs from on-disk file:~n~s~s",
                [
                    SchemaName,
        ...
        _ when Verbosity =:= 'diff' ->
            io:format(
                "~s has keys that on-disk doesn't:~nin schema: ~s~non disk: ~s~n",
                [
                    SchemaName,
...
maybe_log_diff_error(_, _SchemaName, _E) -> io:format("failed to load ~s: ~p~n", [_SchemaName, _E]).
...
-spec add_defaults(
    kz_term:api_object() | kz_term:ne_binary(), kz_term:ne_binary() | kz_json:object()
) -> kz_term:api_object().
...
validate(<<Schema/binary>>, DataJObj, Options) ->
    ...
    validate(
        SchemaJObj, DataJObj, props:insert_values([{'schema_loader_fun', ?DEFAULT_LOADER}], Options)
    );
...
-spec error_to_jobj(jesse_error:error_reason(), options()) ->
    validation_error().
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, {'external_error', Message}, FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_min_length', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_max_length', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_length', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'not_in_range', FailedValue, FailedKeyPath},
    Options
) ->
    ...
    of
        'true' ->
            error_to_jobj(
                {'data_invalid', FailedSchemaJObj, 'not_minimum', FailedValue, FailedKeyPath},
                Options
            );
        'false' ->
            error_to_jobj(
                {'data_invalid', FailedSchemaJObj, 'not_maximum', FailedValue, FailedKeyPath},
                Options
            )
    end;
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'not_in_enum', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_size', FailedValue, FailedKeyPath},
    Options
) ->
    ...
    case length(FailedValue) of
        N when N < Minimum ->
            error_to_jobj(
                {'data_invalid', FailedSchemaJObj, 'wrong_min_items', FailedValue, FailedKeyPath},
                Options
            );
        N when N > Maximum ->
            error_to_jobj(
                {'data_invalid', FailedSchemaJObj, 'wrong_max_items', FailedValue, FailedKeyPath},
                Options
            )
    end;
error_to_jobj(
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_min_properties', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, {'not_unique', _Item}, FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, 'no_extra_properties_allowed', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, 'no_extra_items_allowed', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'no_match', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, 'missing_required_property', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath ++ [FailedValue],
        <<"required">>,
        kz_json:from_list(
            [
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, 'missing_dependency', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'not_divisible', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'not_allowed', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_type', FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, {'missing_required_property', FailKey}, FailedValue,
        FailedKeyPath},
    Options
) ->
    validation_error(
...
error_to_jobj(
    {'data_invalid', FailedSchemaJObj, 'wrong_format', FailedValue, FailedKeyPath},
    Options
) ->
    validation_error(
        FailedKeyPath,
...
error_to_jobj(
    {'data_invalid', _FailedSchemaJObj, FailMsg, FailedValue, FailedKeyPath},
    Options
) ->
...
error_to_jobj(
    {'schema_invalid', Schema, {'schema_not_found', SchemaName}},
    Options
) ->
    Name =
        case kz_term:to_binary(SchemaName) of
...
-spec depreciated_validation_error(
    kz_json:path(), kz_term:ne_binary(), kz_json:object(), options()
) ->
    validation_error().
...
-spec build_error_message(kz_term:ne_binary(), kz_json:object()) ->
    kz_json:object() | kz_term:ne_binary().
...
flatten(?EMPTY_JSON_OBJECT = Empty) ->
    Empty;
...
flatten_props('undefined', Path, Obj) ->
    flatten_prop(Path, Obj);
...
default_object(Schema) ->
    try validate(kz_json:delete_key(<<"required">>, Schema), kz_json:new()) of
        ...
        {'error', Err} ->
            lager:debug(
                "failed to build default object for ~s due to errors: ~p",
...
filter(JObj, Schema) ->
    ...
    FilteredFlat = kz_json:filter(
        fun({K, _}) -> lists:member(K, Filter) end,
