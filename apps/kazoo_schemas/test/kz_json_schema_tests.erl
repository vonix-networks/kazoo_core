...
add_defaults_test_() ->
    ...
    SchemaJObj = kz_json:decode(
        <<"{\"properties\":{\"key1\":{\"type\":\"string\",\"default\":\"default1\"},\"key2\":{\"type\":\"string\",\"default\":\"default2\"}}}">>
    ),
...
add_sub_defaults_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            <<"external_cid">>,
            kz_json:get_value([<<"caller_id">>, <<"external">>, <<"name">>], WithDefaults)
        ),
        ?_assertEqual(
            <<"emer_default">>,
            kz_json:get_value([<<"caller_id">>, <<"emergency">>, <<"name">>], WithDefaults)
        )
    ].
...
add_sub_defaults_array_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            [<<"PCMU">>, <<"PCMA">>], kz_json:get_value([<<"media">>, <<"codecs">>], Gateway)
        ),
...
validate_sub_array_test_() ->
    ...
    JObj = kz_json:decode(
        <<"{\"gateways\":[{\"name\":\"gateway1\",\"media\":{}, \"codecs\": [\"G999\"]}, {\"name\":\"gateway2\", \"codecs\": [\"G999\"]}, {\"name\":\"gateway3\", \"codecs\": [\"PCMU\", \"G999\"]}]}">>
    ),

    [
        ?_assertMatch(
            {error, [
                {data_invalid, _, not_in_enum, <<"G999">>, [<<"gateways">>, 0, <<"codecs">>, 0]},
...
validate_v3_test_() ->
    ...
    [
        ...
        ?_assertMatch(
            {error, [{data_invalid, _, wrong_size, _, [<<"records">>]}]},
            kz_json_schema:validate(V3SchemaJObj, invalid_task_data2())
        ),
        ?_assertMatch(
            {error, [{data_invalid, _, wrong_length, _, [<<"file_name">>]}]},
            kz_json_schema:validate(V3SchemaJObj, invalid_task_data3())
        ),
        ?_assertMatch(
            {error, [{data_invalid, _, wrong_type, _, [<<"file_name">>]}]},
    ...
    [
        ?_assertEqual(
            kz_json:from_list_recursive([
                {<<"caller_id">>, [{<<"emergency">>, [{<<"name">>, <<"emer_default">>}]}]}
            ]),
            Default
        )
    ].
...
flatten_sms_schema_test() ->
    ...
    JObj = kz_json:from_list_recursive([
        {<<"outbound">>, [
            {<<"options">>, [{<<"default">>, [{<<"delivery_mode">>, 2}, {<<"mandatory">>, true}]}]}
        ]},
...
did_duplication_test() ->
    SrvA = kz_json:from_list([
        {<<"DIDs">>, kz_json:new()},
        {<<"auth">>, kz_json:from_list([{<<"auth_method">>, <<"password">>}])},
        {<<"server_name">>, <<"AAA1">>}
    ]),
    SrvB = kz_json:from_list([
        {<<"DIDs">>, kz_json:new()},
        {<<"auth">>, kz_json:from_list([{<<"auth_method">>, <<"password">>}])},
        {<<"server_name">>, <<"BBB1">>}
    ]),
    SrvC = kz_json:from_list([
        {<<"DIDs">>, kz_json:from_list([{<<"+78121111111">>, kz_json:new()}])},
        {<<"auth">>, kz_json:from_list([{<<"auth_method">>, <<"password">>}])},
        {<<"server_name">>, <<"CCC1">>}
    ]),
...
validate_regexp_test_() ->
    ...
    JObj1 = kz_json:decode(
        <<"{\"list\":[\"should@work.com\", \"this_one@too.com\", \"*@fail.com\"]}">>
    ),
    JObj2 = kz_json:decode(
        <<"{\"list\":[\"should@work.com\", \"this_one@too.com\", \"@wont-fail.com\"]}">>
    ),
    ...
    [
        ?_assertMatch(
            {error, [{data_invalid, _, {external_error, Err}, _, [<<"list">>, 2]}]},
            kz_json_schema:validate(SchemaJObj, JObj1)
        ),
        ?_assertMatch({ok, _}, kz_json_schema:validate(SchemaJObj, JObj2)),
        ?_assertMatch({ok, _}, kz_json_schema:validate(SchemaJObj, JObj3)),
        ?_assertMatch(
            {error, [{data_invalid, _, {external_error, Err}, _, [<<"item">>]}]},
            kz_json_schema:validate(SchemaJObj, JObj4)
        )
    ].
