...
urldecode(URL) -> urldecode(URL, 'legacy').
...
urldecode_percent20(Binary) when is_binary(Binary) ->
    kz_term:to_binary(uri_string:percent_decode(kz_term:to_list(Binary)));
urldecode_percent20(String) when is_list(String) ->
    uri_string:percent_decode(String);
urldecode_percent20(Atom) when is_atom(Atom) ->
    kz_term:to_atom(uri_string:percent_decode(kz_term:to_list(Atom)), 'true').

-spec urlencode(kz_term:text()) -> kz_term:text().
urlencode(URL) -> urlencode(URL, 'legacy').
...
urlencode_percent20(Binary) when is_binary(Binary) ->
    kz_term:to_binary(uri_string:normalize(kz_term:to_list(Binary)));
urlencode_percent20(String) when is_list(String) ->
    uri_string:normalize(String);
urlencode_percent20(Atom) when is_atom(Atom) ->
    kz_term:to_atom(uri_string:normalize(kz_term:to_list(Atom)), 'true');
