...
urldecode_legacy() ->
    [
        ...
        ?_assertEqual(
            <<"()a;b:c@d&=">>, kz_http_util:urldecode(<<"%28%29a%3Bb%3Ac%40d%26%3D">>, 'legacy')
        ),
        ?_assertEqual(
            <<"+$,/?#[]">>, kz_http_util:urldecode(<<"%2B%24%2C%2F%3F%23%5B%5D">>, 'legacy')
        ),
        ?_assertEqual(
            <<"oetnh98(*&!@)#$*)!&*$TOHEDU)(FKQJTHDOE*UF**)(!@#&)~~()~*TOEADHuaoeua+?+OE{?LER?\"SN_<T>:QNJZVWM">>,
            kz_http_util:urldecode(
                <<"oetnh98(*%26!%40)%23%24*)!%26*%24TOHEDU)(FKQJTHDOE*UF**)(!%40%23%26)~~()~*TOEADHuaoeua%2B%3F%2BOE%7B%3FLER%3F%22SN_%3CT%3E%3AQNJZVWM">>,
                'legacy'
            )
        ),
...
urldecode_rfc() ->
    [
        ...
        ?_assertEqual(
            <<"()a;b:c@d&=">>, kz_http_util:urldecode(<<"%28%29a%3Bb%3Ac%40d%26%3D">>, 'rfc')
        ),
        ?_assertEqual(
            <<"+$,/?#[]">>, kz_http_util:urldecode(<<"%2B%24%2C%2F%3F%23%5B%5D">>, 'rfc')
        ),
        ?_assertEqual(
            <<"oetnh98(*&!@)#$*)!&*$TOHEDU)(FKQJTHDOE*UF**)(!@#&)~~()~*TOEADHuaoeua+?+OE{?LER?\"SN_<T>:QNJZVWM">>,
            kz_http_util:urldecode(
...
urlsplit_test_() ->
    [
        ?_assertEqual(
            {<<"">>, <<"">>, <<"/foo">>, <<"">>, <<"bar?baz">>},
            kz_http_util:urlsplit(<<"/foo#bar?baz">>)
        ),
        ?_assertEqual(
            {<<"http">>, {<<"host">>, 345}, <<"/foo">>, <<"">>, <<"bar?baz">>},
            kz_http_util:urlsplit(<<"http://host:345/foo#bar?baz">>)
        ),
        ?_assertEqual(
            {<<"http">>, <<"host">>, <<"">>, <<"">>, <<"">>},
            kz_http_util:urlsplit(<<"http://host">>)
        ),
        ?_assertEqual(
            {<<"">>, <<"">>, <<"/wiki/Category:Fruit">>, <<"">>, <<"">>},
            kz_http_util:urlsplit(<<"/wiki/Category:Fruit">>)
        ),
        ?_assertEqual(
...
urlunsplit_test_() ->
    [
        ?_assertEqual(
            <<"/foo#bar?baz">>,
            kz_http_util:urlunsplit({<<"">>, <<"">>, <<"/foo">>, <<"">>, <<"bar?baz">>})
        ),
        ?_assertEqual(
...
json_to_querystring_test_() ->
    Tests = [
        ...
        {<<"{\"foo\":\"bar\",\"fizz\":\"buzz\",\"arr\":[1,3,5]}">>,
            <<"foo=bar&fizz=buzz&arr[]=1&arr[]=3&arr[]=5">>},
        {<<"{\"Msg-ID\":\"123-abc\"}">>, <<"Msg-ID=123-abc">>},
        {<<"{\"url\":\"http://user:pass@host:port/\"}">>,
            <<"url=http%3A%2F%2Fuser%3Apass%40host%3Aport%2F">>},
...
uri_test_() ->
    [
        ?_assertEqual(
            <<"http://test.com/path1/path2">>,
            kz_http_util:uri(<<"http://test.com">>, [<<"path1">>, <<"path2">>])
        ),
        ?_assertEqual(
            <<"http://192.168.0.1:8888/path1/path2">>,
            kz_http_util:uri(<<"http://192.168.0.1:8888/">>, [<<"path1">>, <<"path2">>])
        ),
        ?_assertEqual(
            <<"http://test.com/path1/path2">>,
            kz_http_util:uri(<<"http://test.com/">>, [<<"path1/">>, <<"path2/">>])
...
resolve_uri_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            <<"http://pivot/script2.php">>,
            kz_http_util:resolve_uri(RawPath, <<"/", Relative/binary>>)
        ),
        ...
        ?_assertEqual(
            <<"http://host/d1/d2/a">>,
            kz_http_util:resolve_uri(<<"http://host/d1/d2/d3/file.ext">>, <<"../.././a">>)
        )
    ].
