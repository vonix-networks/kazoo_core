...
scheme([$: | Rest], Acc, Block, Uri) ->
    case Acc of
        [] ->
            {error, scheme, ?LINE};
        _ ->
            Uri1 = Uri#uri{scheme = kzsip_uri:scheme(lists:reverse(Acc))},
            Rest1 = strip(Rest),
...
domain([$/ | Rest], [], Ip6, Block, Uri) ->
    domain(Rest, [], Ip6, Block, Uri);
domain([Ch | _] = Rest, Acc, Ip6, Block, Uri) when
    Ch == $;; Ch == $?; Ch == $>; Ch == $,; Ch == $/
->
...
opts_value([Ch | _] = Rest, Key, Acc, false, _InnerQuoted, Block, Uri) when
    Ch == $;; Ch == $?; Ch == $>; Ch == $,
->
    case Acc of
        [] ->
...
opts_value([Ch | _] = Rest, Key, Acc, false, InnerQuoted, Block, Uri) when
    Ch == 32; Ch == 9; Ch == 13
->
    case strip(Rest) of
        [] ->
...
uri3_test() ->
    ...
    [
        ...
        uris(
            "<sip:xxxx@192.168.1.2:5060;transport=TCP;ob>;reg-id=1;"
            "+sip.instance=\"<urn:uuid:00000000-0000-0000-0000-000097ee887e>\""
        ),
    [
        #uri{
...
uri6_test() ->
    ...
    [#uri{user = <<"u">>, pass = <<"p">>, domain = <<"a">>, port = 20, path = <<"/ws">>}] = uris(
        "<sip:u:p@a:20/ws>"
    ),
    [#uri{domain = <<"a">>, port = 0, path = <<"/ws">>, ext_opts = [<<"b">>]}] = uris(
        "sip: a/ws;b"
    ),
    [
        #uri{
