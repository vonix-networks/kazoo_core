...
add_broker(Broker, Zone) ->
    case kz_amqp_connections:new(Broker, Zone) of
        {'error', 'exists'} ->
            io:format(
                "ERROR: broker ~s currently has ~p connection(s) of which ~p is/are available~n",
                [
                    Broker,
            ...
            io:format(
                "~nTo add more connections use:~n sup kazoo_amqp_maintenance add_connection ~s~n",
                [Broker]
            ),
            'no_return';
        {'error', Reason} ->
            io:format(
                "ERROR: unable to add broker ~s: ~p~n",
                [Broker, Reason]
            ),
            'no_return';
        #kz_amqp_connection{} ->
...
validate_assignments('$end_of_table') ->
    'ok';
validate_assignments({
    [
        #kz_amqp_assignment{
    ...
    is_pid(Channel), is_reference(ChannelRef), is_pid(Connection)
->
    %% validate prechannel
    _ =
        case
            is_process_alive(Channel) andalso
...
validate_assignments({
    ...
    is_pid(Consumer), is_reference(ConsumerRef)
->
...
validate_assignments({
    ...
    is_pid(Consumer),
    is_reference(ConsumerRef),
    is_pid(Channel),
    is_reference(ChannelRef),
    is_pid(Connection),
    is_integer(Assigned)
->
    %% validate assignment
    _ =
        case
            is_process_alive(Consumer) andalso
                is_process_alive(Channel) andalso
                is_process_alive(Connection)
        of
            'false' -> log_invalid_assignment(Assignment);
...
connection_summary() ->
    ...
    io:format(
        "| Broker                                           |    Connection    | Channels | Available | Zone       | Primary |~n"
    ),
    io:format(
        "+==================================================+==================+==========+===========+============+=========+~n"
    ),
...
connection_summary('$end_of_table', _) ->
    'ok';
connection_summary(
    {
        [
...
broker_summary() ->
    ...
    io:format(
        "| Broker                                           | Connections |    Sticky Channels    |    Float Channels     | Prechannels |~n"
    ),
    io:format(
        "|                                                  |             | Assigned | Unassigned | Assigned | Unassigned |             |~n"
    ),
    io:format(
        "+==================================================+=============+==========+============+==========+============+=============+~n"
    ),
...
channel_summary() ->
    ...
    io:format(
        "| Broker                                           |   Age    | Assigned |      Application     |     Consumer    |     Channel     |   Connection    |   Type   | Watchers |~n"
    ),
    io:format(
        "+==================================================+==========+==========+======================+=================+=================+=================+==========+==========+~n"
    ),
...
channel_summary('$end_of_table') ->
    'ok';
channel_summary({[#kz_amqp_assignment{} = Assignment], Continuation}) ->
    io:format(
        "| ~-48s | ~-8B | ~-8B | ~-20w | ~-15w | ~-15w | ~-15w | ~-8s | ~-8B |~n",
        [
...
print_consumer_details('$end_of_table') ->
    'ok';
...
print_consumer_details(Consumer) when is_pid(Consumer) ->
    ...
    _ =
        case kz_amqp_assignments:find(Consumer) of
            {'error', _} ->
                io:format(
                    "  ~-10s: Not currently assigned AMQP channel!~n",
                    ["Status"]
                );
            #kz_amqp_assignment{
                channel = 'undefined',
                ...
                io:format(
                    "  ~-10s: Waiting for sticky channel from ~s~n",
                    ["Status", Broker]
                );
            #kz_amqp_assignment{
                channel = 'undefined',
                ...
                io:format(
                    "  ~-10s: Waiting for float channel from any broker~n",
                    ["Status"]
                );
            #kz_amqp_assignment{
                broker = Broker,
...
print_gc_summary(Results) ->
    ...
    Summary = kz_binary:join(
        [
            kz_util:pretty_print_bytes(kz_term:words_to_bytes(Min), 'truncated'),
...
gc_summary([{_W, Diff, _B, _A} | Results]) ->
    lists:foldl(
        fun gc_summary_fold/2,
        {Diff, Diff, Diff, 1},
        Results
    ).

-spec gc_summary_fold(
...
gc_summary_fold(
    ...
    {lists:min([Diff, Min]), lists:max([Diff, Max]), Diff + Sum, Count + 1}.
