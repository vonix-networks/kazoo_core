...
prop_pretty_print_bytes() ->
    ?FORALL(
        ...
        begin
            ...
            ?WHENFAIL(
                io:format("~pT ~pG ~pM ~pK ~pB (~pb): ~p =:= ~p~n", [
                    T, G, M, K, B, Bytes, Result, Expected
                ]),
                Result =:= Expected
            )
        end
...
proper_test_() ->
    {"Runs the module's PropEr tests during eunit testing",
        {'timeout', 20000, [?_assertEqual([], proper:module(?MODULE, [{'to_file', 'user'}]))]}}.
...
iolist_join_test_() ->
    [
        ...
        ?_assertEqual(
            [$a, <<" || ">>, $b, <<" || ">>, $c], kz_util:iolist_join(<<" || ">>, [$a, $b, $c])
        )
    ].
...
get_event_type_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            {<<"call">>, undefined}, kz_util:get_event_type(kz_json:from_list([EventCategory]))
        ),
        ?_assertEqual({undefined, <<"CHANNEL_CONNECTED">>}, kz_util:get_event_type([EventName])),
        ?_assertEqual(
            {undefined, <<"CHANNEL_CONNECTED">>},
            kz_util:get_event_type(kz_json:from_list([EventName]))
        ),
        ?_assertEqual(
            {<<"call">>, <<"CHANNEL_CONNECTED">>},
            kz_util:get_event_type([EventCategory, EventName])
        ),
        ?_assertEqual(
            {<<"call">>, <<"CHANNEL_CONNECTED">>},
            kz_util:get_event_type(kz_json:from_list([EventCategory, EventName]))
        )
    ].
...
put_callid_test_() ->
    ...
    [
        ?_assertEqual(<<"bla">>, begin
            kz_util:put_callid(<<"bla">>),
            kz_util:get_callid()
        end),
        ?_assertEqual(bla, begin
            kz_util:put_callid(bla),
        ...
        ?_assertEqual(<<"bla">>, begin
            kz_util:put_callid(ApiCallId),
            kz_util:get_callid()
        end),
        ?_assertEqual(<<"bla">>, begin
            kz_util:put_callid(kz_json:from_list(ApiCallId)),
            kz_util:get_callid()
        end),
        ?_assert(
            is_integer(begin
...
account_formats_test_() ->
    ...
    Funs = [
        {fun kz_util:format_account_id/1, AccountId},
    ...
    [
        {format_title(Fun, Format, Expected), format_assert(Fun, Format, Expected)}
     || {Fun, Expected} <- Funs,
        Format <- Formats
    ] ++
        [
...
format_assert(Fun, Format, Expected) ->
    ...
    case {is_simple_modb_converter(Matchable), Format} of
        {'true', ?MATCH_ACCOUNT_RAW(_)} ->
            ?_assertException('error', 'function_clause', Fun(Format));
        {'true', ?MATCH_ACCOUNT_ENCODED(_)} ->
            ?_assertException('error', 'function_clause', Fun(Format));
        {'true', ?MATCH_ACCOUNT_UNENCODED(_)} ->
            ?_assertException('error', 'function_clause', Fun(Format));
        {_Else, Format} ->
...
is_simple_modb_converter("fun kz_util:format_account_modb/1" ++ _) ->
    'true';
is_simple_modb_converter("fun " ?MODULE_STRING ":format_account_modb_raw/1" ++ _) ->
    'true';
is_simple_modb_converter("fun " ?MODULE_STRING ":format_account_modb_encoded/1" ++ _) ->
    'true';
is_simple_modb_converter("fun " ?MODULE_STRING ":format_account_modb_unencoded/1" ++ _) ->
    'true';
is_simple_modb_converter(_Else) ->
    'false'.
...
pretty_print_bytes_test_() ->
    ...
    [
        ?_assertEqual(
            {Bytes, FullFormatted, TruncFormatted},
            {Bytes, kz_util:pretty_print_bytes(Bytes, 'full'),
