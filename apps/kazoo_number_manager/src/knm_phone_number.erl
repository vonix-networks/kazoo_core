...
-export([
    ...
    features_allowed/1,
    features_denied/1,
    add_allowed_feature/2,
    remove_allowed_feature/2,
    add_denied_feature/2,
    remove_denied_feature/2,
    remove_denied_features/1,
    state/1,
    set_state/2,
    reserve_history/1,
    add_reserve_history/2,
    push_reserve_history/1,
    unwind_reserve_history/1,
    ported_in/1,
    set_ported_in/2,
    module_name/1,
    set_module_name/2,
...
-spec from_number_with_options(kz_term:ne_binary(), knm_number_options:options()) ->
    knm_phone_number().
...
bulk_change_fold(JObj, T, Db, PNsMap, ErrorF) ->
    ...
    case
        kz_json:is_true(<<"ok">>, JObj) =:= 'true' orelse
            kz_doc:revision(JObj) =/= 'undefined'
    of
        'true' ->
...
%% @end
-spec handle_bulk_change(
    kz_term:ne_binary(), kz_json:objects(), list() | map(), map(), 'save' | 'delete', fun()
) ->
    map().
handle_bulk_change(Db, JObjs, PNsMap, T0, SaveType, ErrorF) when
    is_map(PNsMap)
->
...
-spec split_by_assignedto_fold(knm_phone_number(), #{
    kz_term:api_ne_binary() => [knm_phone_number()]
}) ->
    #{kz_term:api_ne_binary() => [knm_phone_number()]}.
...
split_by_prev_assigned_to_fold(PN, M, PrevAssignedTo, PrevAssignedTo) ->
    ?LOG_DEBUG(
        "~s prev_assigned_to is same as assigned_to,"
        " not unassign-ing from prev",
        [number(PN)]
    ),
    M#{'undefined' => [PN | maps:get('undefined', M, [])]};
...
handle_fetch(JObj, Options) ->
    ...
    case
        state(PN) =:= ?NUMBER_STATE_AVAILABLE orelse
            is_authorized(PN) orelse
            is_reserved_from_parent(PN)
    of
        'true' -> {'ok', PN};
...
save(T0) ->
    ...
    Tb = knm_numbers:pipe(T, [
        fun is_mdn_for_mdn_run/1,
...
take_not_to_save_fold(PN, {T, NotToSave}) ->
    ...
    case
        NotDirty orelse
            ?NUMBER_STATE_DELETED =:= state(PN)
    of
        'false' ->
            {knm_numbers:ok(PN, T), NotToSave};
        'true' ->
    ...
    {'ok', PN} =
        setters(
            ...
            [
                ...
                {
                    fun set_reserve_history/2,
                    kz_json:get_value(?PVT_RESERVE_HISTORY, JObj, ?DEFAULT_RESERVE_HISTORY)
                },
                ...
                {
                    fun set_module_name/2,
                    kz_json:get_value(?PVT_MODULE_NAME, JObj, ?DEFAULT_MODULE_NAME)
                },
                {
                    fun set_carrier_data/2,
                    kz_json:get_value(?PVT_CARRIER_DATA, JObj, ?DEFAULT_CARRIER_DATA)
                },
                {fun set_region/2, kz_json:get_value(?PVT_REGION, JObj)},
                {fun set_auth_by/2, kz_json:get_value(?PVT_AUTH_BY, JObj)},
                {
                    fun set_features_allowed/2,
                    kz_json:get_list_value(?PVT_FEATURES_ALLOWED, JObj, ?DEFAULT_FEATURES_ALLOWED)
                },
                {
                    fun set_features_denied/2,
                    kz_json:get_list_value(?PVT_FEATURES_DENIED, JObj, ?DEFAULT_FEATURES_DENIED)
                },

                fun ensure_features_defined/1,
                {
                    fun ensure_pvt_state_legacy_undefined/2,
                    kz_json:get_value(?PVT_STATE_LEGACY, JObj)
                }
...
maybe_migrate_features(PN, 'undefined') ->
    reset_features(PN);
maybe_migrate_features(PN, FeaturesList) when
    is_list(FeaturesList)
->
...
ensure_features_defined(PN) ->
    PN.

ensure_pvt_state_legacy_undefined(PN, 'undefined') ->
    PN;
...
maybe_rename_features(Features) ->
    ...
    case
        {
            kz_json:get_ne_value(?LEGACY_DASH_E911, Features),
...
maybe_rename_public_features(JObj) ->
    case
        {
            kz_json:get_ne_value(?LEGACY_DASH_E911, JObj),
...
from_json_with_options(JObj, Options) when
    is_list(Options)
->
    Updates = [
...
-type set_function() ::
    fun((knm_phone_number()) -> setter_acc())
...
set_number(PN, <<"+", _:8, _/binary>> = NormalizedNum) ->
    ...
    case {PN#knm_phone_number.number, PN#knm_phone_number.number_db} of
        ...
        {NormalizedNum, NumberDb} ->
            PN;
        _ ->
            ?DIRTY(PN#knm_phone_number{
                number = NormalizedNum,
                number_db = NumberDb
            })
    end.
...
set_assign_to(PN, AssignTo = undefined) -> PN#knm_phone_number{assign_to = AssignTo};
set_assign_to(PN, AssignTo = ?MATCH_ACCOUNT_RAW(_)) -> PN#knm_phone_number{assign_to = AssignTo}.
...
set_assigned_to(PN = #knm_phone_number{assigned_to = V}, V) ->
    PN;
...
-spec set_assigned_to(knm_phone_number(), kz_term:api_ne_binary(), kz_term:api_ne_binary()) ->
    knm_phone_number().
...
set_prev_assigned_to(
    PN = #knm_phone_number{prev_assigned_to = 'undefined'},
    PrevAssignedTo = ?MATCH_ACCOUNT_RAW(_)
) ->
    PN#knm_phone_number{prev_assigned_to = PrevAssignedTo};
set_prev_assigned_to(PN, 'undefined') ->
    PN;
set_prev_assigned_to(PN = #knm_phone_number{prev_assigned_to = V}, V) ->
    PN;
...
set_used_by(PN = #knm_phone_number{used_by = V}, V) ->
    PN;
...
set_feature(PN0, Feature = ?NE_BINARY, Data) ->
    Features =
        case PN0#knm_phone_number.features of
...
set_state(PN = #knm_phone_number{state = V}, V) ->
    PN;
...
set_reserve_history(PN = #knm_phone_number{reserve_history = V}, V) ->
    PN;
set_reserve_history(PN0 = #knm_phone_number{reserve_history = 'undefined'}, History) when
    is_list(History)
->
    PN1 = PN0#knm_phone_number{reserve_history = ?DEFAULT_RESERVE_HISTORY},
    PN2 = lists:foldr(fun add_reserve_history/2, PN1, History),
    case
        not PN0#knm_phone_number.is_dirty andalso
            PN2#knm_phone_number.is_dirty andalso
            History =:= PN2#knm_phone_number.reserve_history
    of
        'false' ->
            PN2;
        ...
        'true' ->
            ...
            PN2#knm_phone_number{
                is_dirty = 'false',
                modified = PN0#knm_phone_number.modified
            }
    end;
set_reserve_history(PN0, History) when
    is_list(History)
->
    PN1 = PN0#knm_phone_number{reserve_history = ?DEFAULT_RESERVE_HISTORY},
...
add_reserve_history(undefined, PN) ->
    PN;
add_reserve_history(
...
set_ported_in(PN = #knm_phone_number{ported_in = V}, V) ->
    PN;
set_ported_in(PN = #knm_phone_number{ported_in = 'undefined'}, Ported) when
    is_boolean(Ported)
->
...
set_module_name(PN, ?CARRIER_MDN = Name) ->
    set_module_name_local(PN, Name);
set_module_name(PN = #knm_phone_number{module_name = Name}, Name = ?NE_BINARY) ->
    PN;
set_module_name(
    PN = #knm_phone_number{module_name = 'undefined', features = 'undefined'},
    Name = ?NE_BINARY
) ->
    %% Only during from_json/1
    PN#knm_phone_number{module_name = Name};
set_module_name(
    PN0 = #knm_phone_number{module_name = 'undefined', features = Features},
    Name = ?NE_BINARY
) ->
...
set_module_name_local(PN = #knm_phone_number{module_name = Name}, Name) ->
    PN;
...
set_region(PN = #knm_phone_number{region = V}, V) ->
    PN;
...
is_admin(_) -> 'false'.
-else.
is_admin(#knm_phone_number{auth_by = AuthBy}) ->
    is_admin(AuthBy);
...
-spec set_modified(knm_phone_number(), kz_time:gregorian_seconds() | 'undefined') ->
    knm_phone_number().
set_modified(PN = #knm_phone_number{modified = 'undefined'}, 'undefined') ->
    ?DIRTY(PN#knm_phone_number{modified = kz_time:now_s()});
set_modified(PN = #knm_phone_number{modified = V}, V) ->
    PN;
set_modified(PN, Modified) when
    is_integer(Modified), Modified > 0
->
...
set_created(PN = #knm_phone_number{created = 'undefined'}, Created) when
...
set_created(PN = #knm_phone_number{created = V}, V) ->
    PN;
set_created(PN, Created) when
    is_integer(Created), Created > 0
->
...
remove_denied_features(PN) ->
    ...
    ?LOG_WARNING(
        "removing out of sync pvt features: ~s",
        [kz_util:iolist_join($,, lists:usort([ToRm || [ToRm | _] <- RemoveFromPvt]))]
    ),
    ?LOG_WARNING(
        "removing out of sync pub features: ~s",
...
private_to_public() ->
    ...
    PrependPub = [
        [?FEATURE_PREPEND, ?PREPEND_ENABLED],
        [?FEATURE_PREPEND, ?PREPEND_NAME],
        [?FEATURE_PREPEND, ?PREPEND_NUMBER]
    ],
    FailoverPub = [
        [?FEATURE_FAILOVER, ?FAILOVER_E164],
        [?FEATURE_FAILOVER, ?FAILOVER_SIP]
    ],
    RingbackPub = [
        [?FEATURE_RINGBACK, ?RINGBACK_EARLY],
...
%% @end
%%------------------------------------------------------------------------------
-spec list_attachments(knm_phone_number(), kz_term:ne_binary()) ->
    {'ok', kz_json:object()}
    | {'error', any()}.
list_attachments(PN, AuthBy) ->
    AssignedTo = assigned_to(PN),
    case
        state(PN) =:= ?NUMBER_STATE_PORT_IN andalso
...
is_authorized(#knm_phone_number{auth_by = ?KNM_DEFAULT_AUTH_BY}) ->
    'true';
is_authorized(#knm_phone_number{auth_by = 'undefined'}) ->
    'false';
is_authorized(#knm_phone_number{
...
is_reserved_from_parent(#knm_phone_number{
    assigned_to = ?MATCH_ACCOUNT_RAW(AssignedTo),
    auth_by = AuthBy,
    state = ?NUMBER_STATE_RESERVED
}) ->
    Authorized = is_admin_or_in_account_hierarchy(AssignedTo, AuthBy),
    Authorized andalso
        ?LOG_DEBUG("is reserved from parent, allowing"),
    Authorized;
is_reserved_from_parent(_) ->
    'false'.
...
-spec is_reserved_from_parent_collection(knm_numbers:pn_collection()) ->
    knm_numbers:pn_collection().
