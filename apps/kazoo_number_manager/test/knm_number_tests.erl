...
unavailable_tests(ErrorJObj) ->
    [
        {"verify unavailable number error code", ?_assertEqual(403, knm_errors:code(ErrorJObj))},
...
assign_to_app() ->
    ...
    [
        ...
        {"Verify number is now used by MyApp", ?_assertEqual(MyApp, knm_phone_number:used_by(PN1))},
...
fix_number() ->
    ...
    [
        ...
        ?_assert(
            kz_json:are_equal(
                knm_phone_number:feature(PN1, ?FEATURE_E911), kz_json:get_value(?FEATURE_E911, Doc1)
            )
        ),
        ?_assert(
            kz_json:are_equal(
                knm_phone_number:feature(PN1, ?FEATURE_E911),
                kz_json:get_value(?LEGACY_DASH_E911, Doc1)
            )
        ),
        ...
        ?_assert(
            kz_json:are_equal(
                knm_phone_number:feature(PN2, ?FEATURE_E911), kz_json:get_value(?FEATURE_E911, Doc2)
            )
        ),
        ?_assert(
            kz_json:are_equal(
                knm_phone_number:feature(PN2, ?FEATURE_E911),
                kz_json:get_value(?LEGACY_DASH_E911, Doc2)
            )
        ),
        ?_assertEqual(undefined, knm_phone_number:used_by(PN2))
    ].
...
fix_number(N) ->
    ...
    Routines = [
        {fun knm_phone_number:set_used_by/2, UsedBy},
