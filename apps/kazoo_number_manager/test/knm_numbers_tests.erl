...
get_() ->
    ...
    [
        ...
        ?_assertMatch(
            #{ko := #{?NOT_NUM := 'not_reconcilable'}},
            knm_numbers:get([?NOT_NUM], [])
        ),
        ?_assertMatch(
            #{ko := #{?TEST_CREATE_NUM := not_found}},
...
create_() ->
    ...
    [
        ...
        {"Verify feature is properly set",
            ?_assert(
                kz_json:are_equal(E911, knm_phone_number:feature(pn_x(1, Ret), ?FEATURE_E911))
            )},
...
reconcile_() ->
    ...
    Ret2 = knm_numbers:reconcile(
        [?NOT_NUM, ?TEST_AVAILABLE_NUM],
    ...
    [
        ...
        ?_assertEqual(
            #{?NOT_NUM => 'not_reconcilable', ?TEST_AVAILABLE_NUM => error_assign_to_undefined()},
            maps:get(ko, Ret1)
        ),
...
reserve_() ->
    ...
    [
        ...
        ?_assertEqual(
            [?CHILD_ACCOUNT_ID, ?RESELLER_ACCOUNT_ID],
            knm_phone_number:reserve_history(pn_x(1, Ret3))
        )
    ].
