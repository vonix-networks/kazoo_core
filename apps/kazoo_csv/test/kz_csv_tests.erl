...
verifier(_Field, Cell) -> verify_bin1(Cell).
...
take_row_test_() ->
    ...
    [
        ...
        ?_assertEqual({[<<"'a'">>, ?ZILCH], <<>>}, kz_csv:take_row(<<"'a',">>)),
        ?_assertEqual(
            {[?ZILCH, <<"A">>, ?ZILCH, <<"B">>, <<"1, 3">>, ?ZILCH, <<"''">>, ?ZILCH], <<>>},
...
json_to_iolist_test_() ->
    Records1 = [
        kz_json:from_list([{<<"A">>, <<"a1">>}]),
        kz_json:from_list([{<<"A">>, <<"42">>}])
    ],
    Records2 = [
        kz_json:from_list([{<<"field1">>, ?ZILCH}, {<<"field deux">>, <<"QUUX">>}]),
    ...
    Records3 = [
        ...
        kz_json:from_list([
            {<<"account_id">>, <<>>}, {<<"e164">>, <<"+14157215235">>}, {<<"cnam.outbound">>, <<>>}
        ])
    ],
    [
...
parse_test_() ->
    MixedJSONBin =
        <<"a,\"b\",\"\"\"c\"\"\",\"{\"\"data\"\": { \"\"id\"\": \"\"xxxxxxxxx\"\", \"\"int\"\": 20 \"\"bool\"\": false }, \"\"children\"\": {}}\"\n">>,
    MixedJSONResp = [
        <<"a">>,
        <<"b">>,
        <<"\"c\"">>,
    ...
    [
        ...
        ?_assertEqual(
            [<<"\"test\"">>, <<"foo">>], kz_csv:parse_row(<<"\"\"\"test\"\"\",\"foo\"">>)
        ),
        ?_assertEqual(
            [<<"This is a \"test\"">>, <<"foo">>],
            kz_csv:parse_row(<<"\"This is a \"\"test\"\"\",\"foo\"">>)
        ),
        ?_assertEqual(
            [<<"test ,">>, <<"  foo  ">>, <<"bar  ">>],
            kz_csv:parse_row(<<"          \"test ,\"         ,\"  foo  \",\"bar  \"">>)
        ),
        ?_assertEqual([<<"test">>, ?ZILCH, ?ZILCH], kz_csv:parse_row(<<"test,,">>)),
        ?_assertEqual([<<"test">>, ?ZILCH, <<"foo bar">>], kz_csv:parse_row(<<"test,,foo bar">>)),
        ?_assertEqual(
            [?ZILCH, <<"test">>, <<"''">>, <<"foo bar">>], kz_csv:parse_row(<<",test,'',foo bar">>)
        ),
        ?_assertEqual(
            [?ZILCH, <<"test">>, <<>>, <<"foo bar">>], kz_csv:parse_row(<<",test,\"\",foo bar">>)
        ),
