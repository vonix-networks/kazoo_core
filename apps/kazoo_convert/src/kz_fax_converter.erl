...
convert(From, To, Content, Options) when is_map(Options) ->
    case maps:is_key(<<"job_id">>, Options) of
        true ->
            convert(From, To, Content, Options#{<<"from_format">> => From, <<"to_format">> => To});
        false ->
...
image_to_tiff(
    FromPath,
    #{<<"from_format">> := <<"image/tiff">>, <<"tmp_dir">> := TmpDir, <<"job_id">> := JobId} =
        Options
) ->
...
-spec convert_file(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary(), map()) ->
    fax_converted().
...
handle_resample(FromPath, #{<<"tmp_dir">> := TmpDir} = Options) ->
    case
        rename_file(FromPath, filename:join(TmpDir, <<(kz_binary:rand_hex(12))/binary, ".tiff">>))
    of
        {'ok', NewPath} ->
...
-spec select_tiff_command(map()) ->
    ...
    | 'noop'.
select_tiff_command(#{<<"res_x">> := X, <<"res_y">> := Y} = Map) when
    X =:= 0 orelse
        Y =:= 0
->
...
select_tiff_command(#{<<"scheme">> := <<"CCITT Group 3">>, <<"has_pages">> := 'true'} = Map) ->
    lager:debug("file has pages and is valid format for group 3, not going to convert info: ~p", [
        Map
    ]),
    'noop';
select_tiff_command(#{<<"scheme">> := <<"CCITT Group 4">>, <<"has_pages">> := 'true'} = Map) ->
    lager:debug("file has pages and is valid format for group 4, not going to convert info: ~p", [
        Map
    ]),
    'noop';
...
run_validate_command(Command, FromPath, ToPath, TmpDir) ->
    ...
    case kz_os:cmd(Command, Args, Options) of
        {'ok', _} = Ok ->
            ...
            Ok;
        {'error', Reason, Msg} ->
            lager:debug("failed to validate file: ~s with reason: ~s error: ~p", [
                FromPath, Reason, Msg
            ]),
            _ = file:delete(ToPath),
...
count_pages_command(_MimeType, _Filename) ->
    0.
...
rename_file(FromPath, ToPath) ->
    ...
    case filelib:is_file(FromPath) of
        ...
        'false' ->
            {'error', <<"cannot rename from file: ", FromPath/binary, ", file does not exist">>}
    end.
