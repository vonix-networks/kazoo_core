...
-export([
    get_json_value/2, get_json_value/3,
...
decode(JSON, <<"application/json">>) ->
    try
        ...
        _:{'invalid_json', {'error', {_Loc, _Msg}}, _JSON} ->
            lager:error_unsafe("decode error ~s near char # ~b => ~s", [
                _Msg, _Loc, binary:part(JSON, _Loc - 5, 25)
            ]),
            new()
    end.
...
are_equal(JObj1, JObj2) -> to_map(JObj1) =:= to_map(JObj2).
...
-type from_list_options() :: #{
    ascii_list_enforced => boolean(),
...
from_list_recursive([], _) ->
    new();
from_list_recursive(L, Opts) when
    is_list(L)
->
    Options = maps:merge(
        #{
            ascii_list_enforced => 'true',
...
recursive_from_list([First | _] = List, Options) when
    is_list(List), is_tuple(First)
->
    from_list([
        {kz_term:to_binary(K), recursive_from_list(V, Options)}
     || {K, V} <- List
    ]);
...
recursive_from_list({_Y, _M, _D} = Date, _) ->
    kz_date:to_iso8601_extended(Date);
recursive_from_list({{_, _, _}, {_, _, _}} = DateTime, _) ->
    kz_time:iso8601(DateTime);
recursive_from_list(?JSON_WRAPPER(_) = JObj, _) ->
    JObj;
recursive_from_list(_Else, #{invalid_as_null := 'true'}) ->
    'null';
...
-type merge_fun() ::
    fun((key(), merge_arg_2()) -> merge_fun_result())
...
merge(MergeFun, [LeftJObj | RightJObjs]) when is_function(MergeFun, 2) ->
    lists:foldl(
        fun(Right, Left) -> merge(MergeFun, Left, Right) end,
        LeftJObj,
        RightJObjs
    );
...
-spec merge
    (merge_fun(), object(), object()) -> object();
...
merge(MergeFun, [LeftJObj | RightJObjs], Options) when is_function(MergeFun, 2) ->
    lists:foldl(
        fun(Right, Left) -> merge(MergeFun, Left, Right, Options) end,
        LeftJObj,
        RightJObjs
    );
...
merge(MergeFun, [{KX, VX} | Xs] = Left, [{KY, VY} | Ys] = Right, Acc, Options) when
    is_function(MergeFun, 2)
->
    if
...
merge(MergeFun, [{KX, VX} | Xs] = Left, [{KY, VY} | Ys] = Right, Acc, Options) when
    is_function(MergeFun, 3)
->
    if
        KX < KY ->
            merge(MergeFun, Xs, Right, f(KX, MergeFun(KX, {'left', VX}, Options), Acc), Options);
        KX > KY ->
            merge(MergeFun, Left, Ys, f(KY, MergeFun(KY, {'right', VY}, Options), Acc), Options);
        KX =:= KY ->
            merge(MergeFun, Xs, Ys, f(KX, MergeFun(KX, {'both', VX, VY}, Options), Acc), Options)
    end.
...
merge_left(_K, {_Dir, 'null'}, #{'keep_null' := 'true'}) ->
    {'ok', 'null'};
merge_left(_K, {_Dir, 'null'}, _Options) ->
    'undefined';
merge_left(_K, {_Dir, 'undefined'}, _Options) ->
    'undefined';
merge_left(_K, {'both', 'null', _Right}, #{'keep_null' := 'true'}) ->
    {'ok', 'null'};
merge_left(_K, {'both', 'null', _Right}, _Options) ->
    'undefined';
merge_left(_K, {'both', 'undefined', _Right}, _Options) ->
    'undefined';
merge_left(_K, {'left', ?JSON_WRAPPER(_) = Left}, Options) ->
    {'ok', merge(fun merge_left/3, Left, new(), Options)};
merge_left(_K, {'left', V}, _Options) ->
    {'ok', V};
merge_left(_K, {'right', ?JSON_WRAPPER(_) = Right}, Options) ->
    {'ok', merge(fun merge_left/3, Right, new(), Options)};
merge_left(_K, {'right', V}, _Options) ->
    {'ok', V};
...
merge_left(_K, {'both', Left, _Right}, _Options) ->
    {'ok', Left}.
...
merge_right(_K, {_Dir, 'null'}, #{'keep_null' := 'true'}) ->
    {'ok', 'null'};
merge_right(_K, {_Dir, 'null'}, _Options) ->
    'undefined';
merge_right(_K, {_Dir, 'undefined'}, _Options) ->
    'undefined';
merge_right(_K, {'both', _Left, 'null'}, #{'keep_null' := 'true'}) ->
    {'ok', 'null'};
merge_right(_K, {'both', _Left, 'null'}, _Options) ->
    'undefined';
merge_right(_K, {'both', _Left, 'undefined'}, _Options) ->
    'undefined';
merge_right(_K, {'left', ?JSON_WRAPPER(_) = Left}, Options) ->
    {'ok', merge(fun merge_right/3, new(), Left, Options)};
merge_right(_K, {'left', V}, _Options) ->
    {'ok', V};
merge_right(_K, {'right', ?JSON_WRAPPER(_) = Right}, Options) ->
    {'ok', merge(fun merge_right/3, new(), Right, Options)};
merge_right(_K, {'right', V}, _Options) ->
    {'ok', V};
...
merge_right(_K, {'both', _Left, Right}, _Options) ->
    {'ok', Right}.
...
-spec merge_recursive
    (objects(), merge_pred(), merge_options()) -> object();
...
-spec merge_recursive(object(), object() | json_term(), merge_pred(), keys() | [], merge_options()) ->
    object().
merge_recursive(?JSON_WRAPPER(_) = JObj1, ?JSON_WRAPPER(_) = JObj2, Pred, Keys, Options) when
    is_function(Pred, 2)
->
    foldl(
        fun(Key2, Value2, JObj1Acc) ->
...
default_sumer(Value1, undefined) ->
    Value1;
default_sumer(undefined, Value2) ->
    Value2;
default_sumer(Value1, Value2) when
    is_number(Value1),
    is_number(Value2)
->
    Value1 + Value2;
...
sum(?JSON_WRAPPER(_) = JObj1, ?JSON_WRAPPER(_) = JObj2, Sumer) when
    is_function(Sumer, 2)
->
    sum(JObj1, JObj2, Sumer, []).

-spec sum(object(), object(), sumer(), keys() | []) -> object().
sum(?JSON_WRAPPER(_) = JObj1, ?JSON_WRAPPER(_) = JObj2, Sumer, Keys) when
    is_function(Sumer, 2)
->
    F = fun(Key2, Value2, JObj1Acc) -> sum(JObj1Acc, Value2, Sumer, [Key2 | Keys]) end,
    foldl(F, JObj1, JObj2);
sum(?JSON_WRAPPER(_) = JObj1, Value, Sumer, Keys) when
    is_function(Sumer, 2)
->
    Syek = lists:reverse(Keys),
...
sum_jobjs([], Sumer) when
    is_function(Sumer, 2)
->
    new();
sum_jobjs([?JSON_WRAPPER(_) = JObj], Sumer) when
    is_function(Sumer, 2)
->
    JObj;
sum_jobjs([FirstJObj | JObjs], Sumer) when
    is_function(Sumer, 2)
->
...
order_by(Path, Ids, ListOfJObjs) when
    is_list(Ids), is_list(ListOfJObjs)
->
    _ = [
        [put(get_value(Path, JObj), JObj) || JObj <- JObjs]
     || JObjs <- ListOfJObjs
    ],
    [erase(Id) || Id <- Ids].
...
-spec lift_common_properties(objects(), [key() | path()]) ->
    {object(), objects()}.
lift_common_properties([], _Unliftable) ->
    {new(), []};
lift_common_properties([JObj], _Unliftable) ->
    {new(), [JObj]};
lift_common_properties([JObj | JObjs], Unliftable) ->
    ...
    {expand(CommonProperties), remove_common_properties([JObj | JObjs], CommonProperties)}.

-spec lift_common_properties(objects(), [key() | path()], flat_proplist()) -> flat_object().
lift_common_properties(_JObjs, _Unliftable, []) ->
    new();
...
intersection(CommonProperties, JObjProperties) ->
    sets:to_list(
        sets:intersection(sets:from_list(CommonProperties), sets:from_list(JObjProperties))
    ).
...
remove_unliftable([_ | _] = Path, Properties) ->
    lists:filter(
        fun(Property) -> should_remove_unliftable(Property, Path) end,
        Properties
    );
...
filter(Pred, ?JSON_WRAPPER(_) = JObj, Path) when
    is_list(Path),
    is_function(Pred, 1)
->
...
-spec get_ne_binary_value(get_key(), object() | objects(), Default) ->
...
find_first_defined([], _JObjs, Default) ->
    Default;
...
find_value(_Key, _Value, [], Default) ->
    Default;
...
get_first_defined([], _JObj, Default) ->
    Default;
...
-type set_value_fun() ::
    {fun((object(), json_term()) -> object()), json_term()}
    | fun((object()) -> object()).
...
set_values(KVs, JObj, Options) when is_list(KVs) ->
    lists:foldr(
        fun(KV, J) -> set_value_fold(KV, J, Options) end,
        JObj,
        KVs
    ).
...
-spec set_value(get_key(), api_json_term() | 'null', object() | objects(), set_value_options) ->
    object() | objects().
set_value(_Keys, 'undefined', JObj, _Options) ->
    JObj;
...
-spec set_value1(keys(), json_term() | 'null', object() | objects(), set_value_options()) ->
    object() | objects().
...
set_value1([_ | _] = Keys, 'null', JObj, #{'keep_null' := 'false'}) ->
    delete_key(Keys, JObj);
set_value1([_ | _] = Keys, 'undefined', JObj, _Options) ->
    delete_key(Keys, JObj);
set_value1([Key1 | T], Value, ?JSON_WRAPPER(Props), Options) ->
    case lists:keyfind(Key1, 1, Props) of
        {Key1, ?JSON_WRAPPER(_) = V1} ->
            ...
            ?JSON_WRAPPER(
                lists:keyreplace(Key1, 1, Props, {Key1, set_value1(T, Value, V1, Options)})
            );
        ...
        {Key1, _} ->
            ...
            ?JSON_WRAPPER(
                lists:keyreplace(Key1, 1, Props, {Key1, set_value1(T, Value, new(), Options)})
            );
        'false' when T == [] ->
...
set_value1([], Value, _JObj, _Options) ->
    Value.
...
prune_keys(Keys, JObj) when is_list(Keys) ->
    lists:foldr(
        fun(K, JObj0) -> delete_key(K, JObj0, 'prune') end,
        JObj,
        Keys
    ).
...
load_fixture_from_file(App, Dir, File) ->
    Path = list_to_binary([
        code:priv_dir(App), "/", kz_term:to_list(Dir), "/", kz_term:to_list(File)
    ]),
...
% from string:to_lower
normalize_key_char(C) when is_integer(C), 16#C0 =< C, C =< 16#D6 -> C + 32;
% so we only loop once
normalize_key_char(C) when is_integer(C), 16#D8 =< C, C =< 16#DE -> C + 32;
normalize_key_char(C) -> C.

-type search_replace_format() ::
...
flatten_key(K, V = ?EMPTY_JSON_OBJECT) ->
    [{[K], V}];
