...
-type collect_until_acc_fun() :: fun(
    (kz_json:objects(), collect_until_acc()) -> boolean() | {boolean(), collect_until_acc()}
).
-type collect_until_fun() ::
    fun((kz_json:objects()) -> boolean())
...
-type collect_until() ::
    collect_until_fun()
    | whapp()
    %% {Whapp, VFun | IncludeFederated}
    | {whapp(), validate_fun() | boolean()}
    %% {Whapp, VFun, IncludeFederated}
    | {whapp(), validate_fun(), boolean()}
    %% {Whapp, IncludeFederated, IsShared}
    | {whapp(), boolean(), boolean()}
    %% {Whapp, VFun, IncludeFederated, IsShared}
    | {whapp(), validate_fun(), boolean(), boolean()}.
...
-record(state, {
    current_msg_id :: kz_term:api_binary(),
...
maybe_queuename(Args) ->
    case props:get_value('amqp_queuename_start', Args) of
        'undefined' ->
            ?QUEUE_NAME;
        QueueStart ->
            <<(kz_term:to_binary(QueueStart))/binary, "_", (kz_binary:rand_hex(4))/binary>>
    end.
...
next_worker(_Pool, 'undefined') ->
    lager:warning(
        "pool ~s not available yet for application ~s",
        [
            _Pool,
...
next_worker(Pool, Pid) when is_pid(Pid) ->
    try poolboy:checkout(Pool, 'false', default_timeout()) of
        ...
        Worker ->
            lager:debug(
                "application ~s checked out worker ~p from pool ~s",
                [
                    kapps_util:get_application(),
                    Worker,
                    Pool
...
-spec call_custom(
    kz_term:api_terms(), publish_fun(), validate_fun(), timeout(), gen_listener:binding()
) ->
    request_return().
...
-spec call_custom(
    kz_term:api_terms(), publish_fun(), validate_fun(), timeout(), gen_listener:binding(), pid()
) ->
    request_return().
...
call_collect(_Req, _PubFun, 'undefined', _Timeout) ->
    ...
    {'ok', []};
call_collect(Req, PubFun, {Whapp, IncludeFederated}, Timeout) when
    (is_atom(Whapp) orelse
        is_binary(Whapp)) andalso
        is_boolean(IncludeFederated)
->
    CollectFromWhapp = collect_from_whapp(Whapp, IncludeFederated),
    call_collect(Req, PubFun, CollectFromWhapp, Timeout);
call_collect(Req, PubFun, {Whapp, VFun}, Timeout) when
    (is_atom(Whapp) orelse
        is_binary(Whapp)) andalso
        is_function(VFun)
->
    ...
    call_collect(Req, PubFun, CollectFromWhapp, Timeout);
call_collect(Req, PubFun, {Whapp, IncludeFederated, IsShared}, Timeout) when
    (is_atom(Whapp) orelse
        is_binary(Whapp)) andalso
        is_boolean(IncludeFederated) andalso
        is_boolean(IsShared)
->
    CollectFromWhapp = collect_from_whapp(Whapp, IncludeFederated, IsShared),
    call_collect(Req, PubFun, CollectFromWhapp, Timeout);
call_collect(Req, PubFun, {Whapp, VFun, IncludeFederated}, Timeout) when
    (is_atom(Whapp) orelse
        is_binary(Whapp)) andalso
        is_function(VFun) andalso
        is_boolean(IncludeFederated)
->
    CollectFromWhapp = collect_from_whapp_or_validate(Whapp, VFun, IncludeFederated),
    call_collect(Req, PubFun, CollectFromWhapp, Timeout);
call_collect(Req, PubFun, {Whapp, VFun, IncludeFederated, IsShared}, Timeout) when
    (is_atom(Whapp) orelse
        is_binary(Whapp)) andalso
        is_function(VFun) andalso
        is_boolean(IncludeFederated) andalso
...
->
    ...
    call_collect(Req, PubFun, CollectFromWhapp, Timeout);
call_collect(Req, PubFun, Whapp, Timeout) when
    is_atom(Whapp) orelse
        is_binary(Whapp)
->
    call_collect(Req, PubFun, collect_from_whapp(Whapp), Timeout);
call_collect(Req, PubFun, UntilFun, Timeout) when
...
-spec call_collect(kz_term:api_terms(), publish_fun(), collect_until(), timeout(), pid()) ->
    request_return().
call_collect(Req, PubFun, {UntilFun, Acc}, Timeout, Worker) when
    is_function(UntilFun, 2)
->
...
cast(Req, PubFun, Worker) when is_pid(Worker) ->
    Prop = maybe_convert_to_proplist(Req),
    try
        gen_listener:call(Worker, {'publish', Prop, PubFun})
    catch
        _E:R ->
            lager:debug("request failed: ~s: ~p", [_E, R]),
            {'error', R}
...
collect_from_whapp(Whapp, IncludeFederated, IsShared) ->
    Count =
        case {IncludeFederated, IsShared} of
            %% Get from {0,1} whapp instance per zone
            {'true', 'true'} -> kz_nodes:whapp_zone_count(Whapp);
            %% Get from one whapp instance
            {'false', 'true'} -> 1;
            %% Get from all instances, either local or federated
            _ -> kz_nodes:whapp_count(Whapp, IncludeFederated)
        end,
...
count_fun(Count) -> fun(Responses) -> length(Responses) >= Count end.
...
-spec collect_from_whapp_or_validate(kz_term:text(), validate_fun(), boolean()) ->
    collect_until_fun().
...
-spec collect_from_whapp_or_validate(kz_term:text(), validate_fun(), boolean(), boolean()) ->
    collect_until_fun().
...
request_proplist_filter({_, 'undefined'}) ->
    'false';
request_proplist_filter(_) ->
    'true'.
...
handle_call(Call, From, #state{queue = 'undefined'} = State) when
    is_tuple(Call)
->
...
handle_call(
    {'request', ReqProp, PublishFun, VFun, Timeout},
    {ClientPid, _} = From,
    ...
    case send_request(CallId, Q, PublishFun, ReqProp) of
        'ok' when Confirm =:= 'true' ->
            lager:debug("published request with msg id ~s for ~p waiting for confirmation", [
                MsgId, ClientPid
            ]),
            {'noreply', State#state{
                ...
                client_from = From,
                client_vfun = VFun,
                % how we know not to collect any responses
                responses = 'undefined',
                neg_resp_count = 0,
...
handle_call(
    {'call_collect', ReqProp, PublishFun, UntilFun, Timeout, Acc},
    {ClientPid, _} = From,
    #state{queue = Q, confirms = Confirm} = State
) ->
    _ = kz_util:put_callid(ReqProp),
    ...
    case send_request(CallId, Q, PublishFun, ReqProp) of
        'ok' when Confirm =:= 'true' ->
            ...
            {'noreply', State#state{
                ...
                acc = Acc,
                % how we know to collect all responses
                responses = [],
                neg_resp_count = 0,
                current_msg_id = MsgId,
...
handle_call(
    ...
    case publish_api(PublishFun, ReqProp) of
        ...
        'ok' ->
            ...
            {'reply', 'ok', State};
        {'error', _E} = Err ->
            lager:error("failed to publish message ~s for ~p: ~p", [
                kz_api:msg_id(ReqProp), _Pid, _E
            ]),
            {'reply', Err, State}
    end;
handle_call(
    {'publish', ReqProp, PublishFun},
    {Pid, _} = From,
...
handle_cast(
    {'gen_listener', {'return', JObj, BasicReturn}},
    #state{
        current_msg_id = MsgId,
        client_from = From,
        confirms = Confirms
...
handle_info(
    {'DOWN', ClientRef, 'process', _Pid, _Reason},
    #state{
        current_msg_id = _MsgId,
...
handle_info(
    ...
    case kz_term:is_empty(ReservedJObj) of
        'true' ->
            lager:debug(
                "negative response threshold reached, returning last negative message to ~p", [_Pid]
            ),
            gen_server:reply(From, {'error', ErrorJObj});
        'false' ->
            lager:debug("negative response threshold reached, returning deferred response to ~p", [
                _Pid
            ]),
...
handle_info(
    ...
    case kz_term:is_empty(ReservedJObj) of
        ...
        'false' ->
            lager:debug("only received deferred response for msg id: ~s and client: ~p", [
                _MsgId, _Pid
            ]),
            gen_server:reply(From, {'ok', ReservedJObj})
    end,
...
handle_info(
    ...
    {'noreply', reset(State), 'hibernate'};
handle_info(
    {'timeout', ReqRef, 'confirm_timeout'},
...
reset(
    ...
    kz_util:put_callid(?DEFAULT_LOG_SYSTEM_ID),
    _ =
        case is_reference(ReqRef) of
            'true' -> erlang:cancel_timer(ReqRef);
            'false' -> 'ok'
        end,
    _ =
        case is_reference(ClientRef) of
            'true' -> erlang:demonitor(ClientRef, ['flush']);
            'false' -> 'ok'
        end,
    _ =
        case is_reference(ConfirmRef) of
...
start_req_timeout(Timeout) -> erlang:start_timer(Timeout, self(), 'req_timeout').

-spec start_confirm_timeout(timeout()) -> reference().
start_confirm_timeout('infinity') -> make_ref();
start_confirm_timeout(Timeout) -> erlang:start_timer(Timeout, self(), 'confirm_timeout').
...
-spec handle_confirm(#'basic.ack'{} | #'basic.nack'{}, state()) ->
    kz_types:handle_cast_ret_state(state()).
...
-spec handle_method_confirm(atom(), #'basic.ack'{} | #'basic.nack'{}, state()) ->
    kz_types:handle_cast_ret_state(state()).
...
-spec handle_payload(kz_term:ne_binary(), kz_json:object(), state()) ->
    kz_types:handle_cast_ret_state(state()).
handle_payload(
    MsgId,
    JObj,
        ...
        'true' ->
            case kz_json:is_true(<<"Defer-Response">>, JObj) of
                'false' ->
                    ...
                    {'noreply', reset(State), 'hibernate'};
                'true' ->
                    lager:debug("deferred response for msg id ~s, waiting for primary response", [
                        MsgId
                    ]),
...
->
    ...
    try UntilFun(Responses, Acc) of
        'true' ->
            ...
            lager:debug(
                "response for msg id ~s took ~bμs to return",
...
handle_payload(
    ...
    try UntilFun(Responses) of
        'true' ->
            lager:debug("responses have apparently met the criteria for the client, returning"),
            lager:debug(
                "response for msg id ~s took ~bus to return",
                [MsgId, kz_time:elapsed_us(StartTime)]
            ),
            gen_server:reply(From, {'ok', Responses}),
            {'noreply', reset(State), 'hibernate'};
